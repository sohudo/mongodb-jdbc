...
public class Pretest {
		    ...
		    public static void main( String args[] )
		        ...
		        /*
		    	String sql = "insert into hotnews(id,title) values(?,?)";
                PreparedStatement  st = c.prepareStatement(sql);
                st.setInt(1, 202137);
                String s = "{\"nt\":\"\\\"TP-LINK_C9BE\\\"\"}";
                st.setString(2, s);
                st.executeUpdate();	 
                
               // {"nt":"\"TP-LINK_C9BE\""} 想存入这个值。结果成了 {"nt":"\\"TP-LINK_C9BE\\""}
                */
		        
		        
		       // select * from firewall where idauto=16600918"));//
		       //st.executeUpdate("CREATE TABLE IF NOT EXISTS src1 (key INT, value STRING)");
