...
//	private   final String _sql;
	...
	public MongoData query() throws MongoSQLException{
        ...
        HashMap<String, String> columnAliases =new HashMap<String, String>();
        DBCursor c=null;
		...
		if(sqlSelectQuery instanceof MySqlSelectQueryBlock) {
			...
			for(SQLSelectItem item : mysqlSelectQuery.getSelectList()) {
				...
				if(StringUtils.isNotBlank(item.getAlias())){
                    columnAliases.put(item.getExpr().toString(), item.getAlias());
                }
			}	
		   ...
		   mongo.setCursor(c);
           mongo.setColumnAliasMap(columnAliases);
		}
		return  mongo;		
	}
